...
session:
  persist: true
  # file: "<STATE_DIR>/session.json" # Optional custom session file
...
### `version`
The schema version for the configuration file. Currently, this should be set to `1`.
...
### `keybinds`
All keybindings are configurable. They follow the standard key string format (e.g., `ctrl+x`, `shift+k`, `ctrl+alt+delete`).
- `toggle_focus`: Swaps focus between the active terminal and the tab list.
...
- `cwd`: (Default: `~`) Initial working directory. Supports `~` and `<CONFIG_DIR>` placeholders.
